...
void eval(char *cmdline) {
  ...
  // 如果不是内置命令，则 fork child 执行
  if (!builtin_cmd(argv)) {
    // 防止 child 在父进程还未添加其 job 前就退出，需要 block SIGCHLD 信号
    ...
    // 因为需要记录 job，所以屏蔽信号以便访问全局变量
    // （因为可能发生竞争的就是几个 sig handler，屏蔽信号后它们无法调用）
    sigset_t mask_all;
    ...
    // 输出 bg 信息或等待 fg
    is_bg ? printf("[%d] (%d) %s", pid2jid(pid), pid, cmdline) : waitfg(pid);
...
void do_bgfg(char **argv) {
  ...
  // non-exist pid or jid
  if (argv[1] == NULL) {
    printf("incorrect argv for pid/jid\n");
    return;
  }
  ...
  if (!is_bg) {
    if (fgpid(jobs)) {
      // 如果当前存在 fg，则不允许执行 fg 指令
      printf("there is a running job, and dont exec fg cmd\n");
      return;
    }

    // 阻塞 SIGCHLD 信号，以免还未执行 waitfg 时，SIGCHLD 信号到来
